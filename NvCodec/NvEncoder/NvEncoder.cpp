...
void NvEncoder::LoadNvEncApi() {
    ...
    if (hModule == NULL) {
        NVENC_THROW_ERROR("NVENC library file is not found. Please ensure NV "
                          "driver is installed",
...
void NvEncoder::CreateDefaultEncoderParams(
    ...
    if (pIntializeParams == nullptr ||
        ...
        NVENC_THROW_ERROR("pInitializeParams and "
...
void NvEncoder::CreateEncoder(const NV_ENC_INITIALIZE_PARAMS *pEncoderParams) {
    ...
    if (pEncoderParams->encodeGUID == NV_ENC_CODEC_H264_GUID) {
        if ((m_eBufferFormat == NV_ENC_BUFFER_FORMAT_YUV444) &&
            (pEncoderParams->encodeConfig->encodeCodecConfig.h264Config
                 .chromaFormatIDC != 3)) {
            NVENC_THROW_ERROR("Invalid ChromaFormatIDC",
                              NV_ENC_ERR_INVALID_PARAM);
        }
    }
    ...
    if (pEncoderParams->encodeGUID == NV_ENC_CODEC_HEVC_GUID) {
        ...
        if (yuv10BitFormat && pEncoderParams->encodeConfig->encodeCodecConfig
            ...
            NVENC_THROW_ERROR("Invalid PixelBitdepth",
                              NV_ENC_ERR_INVALID_PARAM);
        }
    ...
    if (pEncoderParams->encodeConfig) {
        ...
        NV_ENC_PRESET_CONFIG presetConfig = {NV_ENC_PRESET_CONFIG_VER,
                                             {NV_ENC_CONFIG_VER}};
        m_nvenc.nvEncGetEncodePresetConfig(
            m_hEncoder, pEncoderParams->encodeGUID, NV_ENC_PRESET_DEFAULT_GUID,
            &presetConfig);
...
void NvEncoder::EncodeFrame(std::vector<std::vector<uint8_t>> &vPacket,
    ...
    if (!IsHWEncoderInitialized()) {
        NVENC_THROW_ERROR("Encoder device not found",
                          NV_ENC_ERR_NO_ENCODE_DEVICE);
    }
    int i = m_iToSend % m_nEncoderBuffer;
...
void NvEncoder::RunMotionEstimation(std::vector<uint8_t> &mvData) {
    ...
    NVENC_API_CALL(
        m_nvenc.nvEncMapInputResource(m_hEncoder, &mapInputResource));
    NV_ENC_INPUT_PTR pDeviceMemoryInputBufferForReference =
        mapInputResource.mappedResource;
    m_vMappedRefBuffers[i] = mapInputResource.mappedResource;

    DoMotionEstimation(pDeviceMemoryInputBuffer,
                       pDeviceMemoryInputBufferForReference, mvData);
}
...
void NvEncoder::DoEncode(NV_ENC_INPUT_PTR inputBuffer,
    ...
    picParams.outputBitstream =
        m_vBitstreamOutputBuffer[m_iToSend % m_nEncoderBuffer];
    picParams.completionEvent =
        m_vpCompletionEvent[m_iToSend % m_nEncoderBuffer];
...
void NvEncoder::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer,
    ...
    for (; m_iGot < iEnd; m_iGot++) {
        ...
        NVENC_API_CALL(
            m_nvenc.nvEncLockBitstream(m_hEncoder, &lockBitstreamData));
...
bool NvEncoder::Reconfigure(
    ...
    NVENC_API_CALL(m_nvenc.nvEncReconfigureEncoder(
        m_hEncoder,
        const_cast<NV_ENC_RECONFIGURE_PARAMS *>(pReconfigureParams)));
    ...
    if (pReconfigureParams->reInitEncodeParams.encodeConfig) {
        memcpy(&m_encodeConfig,
               pReconfigureParams->reInitEncodeParams.encodeConfig,
               sizeof(m_encodeConfig));
    }
...
void NvEncoder::RegisterResources(std::vector<void *> inputframes,
    ...
    for (uint32_t i = 0; i < inputframes.size(); ++i) {
        ...
        inputframe.numChromaPlanes =
            NvEncoder::GetNumChromaPlanes(bufferFormat);
        ...
        if (bReferenceFrame) {
            m_vRegisteredResourcesForReference.push_back(
                registerResource.registeredResource);
            m_vReferenceFrames.push_back(inputframe);
        } else {
            m_vRegisteredResources.push_back(
                registerResource.registeredResource);
...
void NvEncoder::UnregisterResources() {
    ...
    for (uint32_t i = 0; i < m_vRegisteredResourcesForReference.size(); ++i) {
        if (m_vRegisteredResourcesForReference[i]) {
            m_nvenc.nvEncUnregisterResource(
                m_hEncoder, m_vRegisteredResourcesForReference[i]);
        }
    }
...
void NvEncoder::WaitForCompletionEvent(int iEvent) {
    ...
    if (WaitForSingleObject(m_vpCompletionEvent[iEvent], 20000) ==
        WAIT_FAILED) {
...
void NvEncoder::GetChromaSubPlaneOffsets(
    ...
    case NV_ENC_BUFFER_FORMAT_IYUV:
        chromaOffsets.push_back(pitch * height);
        chromaOffsets.push_back(
            chromaOffsets[0] + (NvEncoder::GetChromaPitch(bufferFormat, pitch) *
...
void NvEncoder::InitializeBitstreamBuffer() {
    for (int i = 0; i < m_nEncoderBuffer; i++) {
        ...
        NVENC_API_CALL(m_nvenc.nvEncCreateBitstreamBuffer(
            m_hEncoder, &createBitstreamBuffer));
...
void NvEncoder::InitializeMVOutputBuffer() {
    for (int i = 0; i < m_nEncoderBuffer; i++) {
        ...
        NVENC_API_CALL(
            m_nvenc.nvEncCreateMVBuffer(m_hEncoder, &createMVBuffer));
