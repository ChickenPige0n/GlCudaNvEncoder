...
void NvEncoderCuda::AllocateInputBuffers(int32_t numInputBuffers) {
    ...
    for (int count = 0; count < numCount; count++) {
        ...
        RegisterResources(inputFrames, NV_ENC_INPUT_RESOURCE_TYPE_CUDADEVICEPTR,
                          GetMaxEncodeWidth(), GetMaxEncodeHeight(),
                          (int)m_cudaPitch, GetPixelFormat(),
                          (count == 1) ? true : false);
    }
}
...
void NvEncoderCuda::CopyToDeviceFrame(CUcontext device, void *pSrcFrame,
    ...
    for (uint32_t i = 0; i < numChromaPlanes; ++i) {
        if (chromaHeight) {
            if (srcMemoryType == CU_MEMORYTYPE_HOST) {
                ...
                m.srcDevice =
                    (CUdeviceptr)((uint8_t *)pSrcFrame + srcChromaOffsets[i]);
            }
            m.srcPitch = srcChromaPitch;

            m.dstDevice =
                (CUdeviceptr)((uint8_t *)pDstFrame + dstChromaOffsets[i]);
