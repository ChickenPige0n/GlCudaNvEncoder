...
NvEncoderD3D11::NvEncoderD3D11(ID3D11Device *pD3D11Device, uint32_t nWidth,
    ...
    if (GetD3D11Format(GetPixelFormat()) == DXGI_FORMAT_UNKNOWN) {
        NVENC_THROW_ERROR("Unsupported Buffer format",
...
NvEncoderD3D11::~NvEncoderD3D11() { ReleaseD3D11Resources(); }
...
void NvEncoderD3D11::ReleaseInputBuffers() { ReleaseD3D11Resources(); }
...
void NvEncoderD3D11::ReleaseD3D11Resources() {
    ...
    for (uint32_t i = 0; i < m_vInputFrames.size(); ++i) {
        if (m_vInputFrames[i].inputPtr) {
            reinterpret_cast<ID3D11Texture2D *>(m_vInputFrames[i].inputPtr)
                ->Release();
        }
    }
