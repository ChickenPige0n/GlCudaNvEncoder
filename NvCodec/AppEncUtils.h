...
inline void ShowHelpAndExit_AppEncD3D(const char *szBadOption = NULL) {
    ...
    oss << "Options:" << std::endl
        ...
        << "-nv12        (No value) Convert to NV12 before encoding. Don't use "
           "it with -444"
        << std::endl;
...
inline void ParseCommandLine_AppEncD3D(int argc, char *argv[],
                                       char *szInputFileName, int &nWidth,
                                       int &nHeight, char *szOutputFileName,
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (!_stricmp(argv[i], "-s")) {
            if (++i == argc ||
                2 != sscanf(argv[i], "%dx%d", &nWidth, &nHeight)) {
