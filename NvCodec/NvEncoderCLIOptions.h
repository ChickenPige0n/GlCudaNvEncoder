...
class NvEncoderInitParam {
  public:
    NvEncoderInitParam(const char *szParam = "",
                       std::function<void(NV_ENC_INITIALIZE_PARAMS *pParams)>
                           *pfuncInit = NULL,
        ...
        for (unsigned i = 0; i < tokens.size(); i++) {
            if (tokens[i] == "-codec" && ++i != tokens.size()) {
                ParseString("-codec", tokens[i], vCodec, szCodecNames,
                            &guidCodec);
                continue;
            }
            if (bLowLatency) {
    ...
    std::string GetHelpMessage(bool bMeOnly = false, bool bUnbuffered = false,
        ...
        oss << "-codec       Codec: " << szCodecNames << std::endl
            << "-preset      Preset: "
            << (bLowLatency ? szLowLatencyPresetNames : szPresetNames)
            << std::endl
            << "-profile     H264: " << szH264ProfileNames
            << "; HEVC: " << szHevcProfileNames << std::endl;
        if (!bHide444 && !bLowLatency) {
            oss << "-444         (Only for RGB input) YUV444 encode"
                << std::endl;
        }
        if (bMeOnly)
            return oss.str();
        ...
        oss << "-bitrate     Average bit rate, can be in unit of 1, K, M"
            ...
            << std::endl
            << "-vbvinit     VBV initial delay in bits, can be in unit of 1, "
               "K, M"
            << std::endl;
        if (!bLowLatency) {
            oss << "-aq          Enable spatial AQ and set its stength (range "
                   "1-15, "
                   "0-auto)"
                << std::endl
        ...
        if (!bLowLatency) {
            oss << "-constqp     QP value for constqp rate control mode"
                << std::endl
        ...
        if (bUnbuffered && !bLowLatency) {
            oss << "Note: Options -bf and -lookahead are unavailable for this "
                   "app"
                << std::endl;
        }
        return oss.str();
    ...
    std::string MainParamToString(const NV_ENC_INITIALIZE_PARAMS *pParams) {
        ...
        os << std::endl
           ...
           << "\tbitrate      : "
           << pParams->encodeConfig->rcParams.averageBitRate << std::endl
           << "\tmaxbitrate   : " << pParams->encodeConfig->rcParams.maxBitRate
           << std::endl
           << "\tvbvbufsize   : "
           << pParams->encodeConfig->rcParams.vbvBufferSize << std::endl
           << "\tvbvinit      : "
           << pParams->encodeConfig->rcParams.vbvInitialDelay << std::endl
           << "\taq           : "
           ...
           << (pParams->encodeConfig->rcParams.enableLookahead
                   ? std::to_string(
                         pParams->encodeConfig->rcParams.lookaheadDepth)
                   : "disabled")
           << std::endl
           << "\tcq           : "
           << pParams->encodeConfig->rcParams.targetQuality << std::endl
           << "\tqmin         : P,B,I="
  ...
  public:
    ...
    virtual void SetInitParams(NV_ENC_INITIALIZE_PARAMS *pParams,
        ...
        for (unsigned i = 0; i < tokens.size(); i++) {
            ...
            errmessage
                << "Re-run the application with the -h option to get a list "
  ...
  private:
    ...
    bool ParseString(const std::string &strName, const std::string &strValue,
        ...
        auto it =
            std::find(vstrValueName.begin(), vstrValueName.end(), strValue);
    ...
    std::string ConvertValueToString(const std::vector<T> &vValue,
                                     const std::string &strValueNames,
                                     T value) {
    ...
    bool ParseQp(const std::string &strName, const std::string &strValue,
        ...
        try {
            if (vQp.size() == 1) {
                ...
                *pQp = {(unsigned)std::stoi(vQp[0]),
                        (unsigned)std::stoi(vQp[1]),
  ...
  private:
    ...
    std::vector<NV_ENC_QP_MAP_MODE> vQpMapMode =
        std::vector<NV_ENC_QP_MAP_MODE>{
  ...
  public:
    ...
    FullParamToString(const NV_ENC_INITIALIZE_PARAMS *pInitializeParams) {
        ...
        os << "NV_ENC_INITIALIZE_PARAMS:" << std::endl
           ...
           << pInitializeParams->enableExternalMEHints << std::endl
           ...
           << "maxEncodeWidth: " << pInitializeParams->maxEncodeWidth
           << std::endl
           << "maxEncodeHeight: " << pInitializeParams->maxEncodeHeight
           << std::endl
        ...
        os << "NV_ENC_CONFIG:" << std::endl
           ...
           << std::endl
           << "    maxBitRate:      " << pConfig->rcParams.maxBitRate
           << std::endl
           ...
           << std::endl
           << "    disableIadapt: " << pConfig->rcParams.disableIadapt
           << std::endl
           << "    disableBadapt: " << pConfig->rcParams.disableBadapt
           << std::endl
           << "    enableTemporalAQ: " << pConfig->rcParams.enableTemporalAQ
           << std::endl
           << "    zeroReorderDelay: " << pConfig->rcParams.zeroReorderDelay
           << std::endl
           << "    enableNonRefP: " << pConfig->rcParams.enableNonRefP
           << std::endl
           ...
           << pConfig->rcParams.temporallayerIdxMask << std::endl
           << "    temporalLayerQP: "
           << (int)pConfig->rcParams.temporalLayerQP[0] << ", "
           ...
           << "    targetQuality: " << pConfig->rcParams.targetQuality
           << std::endl
        ...
        if (pInitializeParams->encodeGUID == NV_ENC_CODEC_H264_GUID) {
            os << "NV_ENC_CODEC_CONFIG (H264):" << std::endl
               << "    enableTemporalSVC: "
               << pConfig->encodeCodecConfig.h264Config.enableTemporalSVC
               << std::endl
               << "    enableStereoMVC: "
               << pConfig->encodeCodecConfig.h264Config.enableStereoMVC
               << std::endl
               << "    hierarchicalPFrames: "
               ...
               << std::endl
               << "    outputAUD: "
               << pConfig->encodeCodecConfig.h264Config.outputAUD << std::endl
               << "    disableSPSPPS: "
               << pConfig->encodeCodecConfig.h264Config.disableSPSPPS
               << std::endl
               ...
               << std::endl
               << "    repeatSPSPPS: "
               << pConfig->encodeCodecConfig.h264Config.repeatSPSPPS
               << std::endl
               << "    enableVFR: "
               << pConfig->encodeCodecConfig.h264Config.enableVFR << std::endl
               << "    enableLTR: "
               << pConfig->encodeCodecConfig.h264Config.enableLTR << std::endl
               ...
               << std::endl
               << "    idrPeriod: "
               << pConfig->encodeCodecConfig.h264Config.idrPeriod << std::endl
               ...
               << std::endl
               << "    numTemporalLayers: "
               << pConfig->encodeCodecConfig.h264Config.numTemporalLayers
               << std::endl
               << "    enableTemporalSVC: "
               << pConfig->encodeCodecConfig.h264Config.enableTemporalSVC
               << std::endl
               ...
               << std::endl
               << "    fmoMode: "
               << pConfig->encodeCodecConfig.h264Config.fmoMode << std::endl
               << "    bdirectMode: "
               << pConfig->encodeCodecConfig.h264Config.bdirectMode << std::endl
               << "    entropyCodingMode: "
               << pConfig->encodeCodecConfig.h264Config.entropyCodingMode
               << std::endl
               << "    stereoMode: "
               ...
               << std::endl
               << "    intraRefreshCnt: "
               << pConfig->encodeCodecConfig.h264Config.intraRefreshCnt
               << std::endl
               << "    maxNumRefFrames: "
               << pConfig->encodeCodecConfig.h264Config.maxNumRefFrames
               << std::endl
               << "    sliceMode: "
               << pConfig->encodeCodecConfig.h264Config.sliceMode << std::endl
               << "    sliceModeData: "
               << pConfig->encodeCodecConfig.h264Config.sliceModeData
               << std::endl
               ...
               << std::endl
               << "    ltrNumFrames: "
               << pConfig->encodeCodecConfig.h264Config.ltrNumFrames
               << std::endl
               << "    ltrTrustMode: "
               << pConfig->encodeCodecConfig.h264Config.ltrTrustMode
               << std::endl
               << "    chromaFormatIDC: "
               << pConfig->encodeCodecConfig.h264Config.chromaFormatIDC
               << std::endl
               << "    maxTemporalLayers: "
            ...
            os << "NV_ENC_CODEC_CONFIG (HEVC):" << std::endl
               ...
               << std::endl
               << "    minCUSize: "
               << pConfig->encodeCodecConfig.hevcConfig.minCUSize << std::endl
               << "    maxCUSize: "
               << pConfig->encodeCodecConfig.hevcConfig.maxCUSize << std::endl
               << "    useConstrainedIntraPred: "
               ...
               << std::endl
               << "    outputAUD: "
               << pConfig->encodeCodecConfig.hevcConfig.outputAUD << std::endl
               << "    enableLTR: "
               << pConfig->encodeCodecConfig.hevcConfig.enableLTR << std::endl
               << "    disableSPSPPS: "
               << pConfig->encodeCodecConfig.hevcConfig.disableSPSPPS
               << std::endl
               << "    repeatSPSPPS: "
               << pConfig->encodeCodecConfig.hevcConfig.repeatSPSPPS
               << std::endl
               << "    enableIntraRefresh: "
               << pConfig->encodeCodecConfig.hevcConfig.enableIntraRefresh
               << std::endl
               << "    chromaFormatIDC: "
               << pConfig->encodeCodecConfig.hevcConfig.chromaFormatIDC
               << std::endl
               << "    pixelBitDepthMinus8: "
               << pConfig->encodeCodecConfig.hevcConfig.pixelBitDepthMinus8
               << std::endl
               << "    idrPeriod: "
               << pConfig->encodeCodecConfig.hevcConfig.idrPeriod << std::endl
               << "    intraRefreshPeriod: "
               << pConfig->encodeCodecConfig.hevcConfig.intraRefreshPeriod
               << std::endl
               << "    intraRefreshCnt: "
               << pConfig->encodeCodecConfig.hevcConfig.intraRefreshCnt
               << std::endl
               << "    maxNumRefFramesInDPB: "
               << pConfig->encodeCodecConfig.hevcConfig.maxNumRefFramesInDPB
               << std::endl
               << "    ltrNumFrames: "
               << pConfig->encodeCodecConfig.hevcConfig.ltrNumFrames
               << std::endl
               ...
               << std::endl
               << "    sliceMode: "
               << pConfig->encodeCodecConfig.hevcConfig.sliceMode << std::endl
               << "    sliceModeData: "
               << pConfig->encodeCodecConfig.hevcConfig.sliceModeData
               << std::endl
               ...
               << std::endl
               << "    ltrTrustMode: "
               << pConfig->encodeCodecConfig.hevcConfig.ltrTrustMode
               << std::endl;
        }
        return os.str();
    }
